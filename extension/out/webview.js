...
let panel;
function openGraphPanel(context, getGraph) {
    ...
    panel = vscode.window.createWebviewPanel("snipeGraph", "Snipe Repo Knowledge Graph", column, { enableScripts: true, retainContextWhenHidden: true });
    panel.onDidDispose(() => (panel = undefined));
    refreshGraphContent(getGraph);
}
async function refreshGraphContent(getGraph) {
    const p = panel;
    if (!p)
        return;
    try {
...
function refreshGraph(getGraph) {
    if (panel)
        refreshGraphContent(getGraph);
}
